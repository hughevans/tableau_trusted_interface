...
RSpec.describe TableauTrustedInterface::Report, vcr: { cassette_name: 'success' } do
  before do
    TableauTrustedInterface.configure do |config|
      ...
      config.default_auth_server = 'https://example.com'
      config.default_view_server = 'https://view.example.com'
      config.default_server = 'http://backup.example.com'
    end
  end
  ...
  context 'when no auth_server or view_server are passed in as options, but server is', vcr: { cassette_name: 'backup' } do
    subject { TableauTrustedInterface::Report.new(server: 'http://backup.example.org', user: 'baz') }

    it 'sets #auth_server from the server option' do
      expect(subject.auth_server).to eql('http://backup.example.org')
    end

    it 'sets #view_server from the view_server option' do
      expect(subject.view_server).to eql('http://backup.example.org')
    end
  end


  context 'when there is no auth_server and view_server in the configuration, but there is a server', vcr: { cassette_name: 'backup' } do
    before do
      TableauTrustedInterface.configure do |config|
        config.default_auth_server = nil
        config.default_view_server = nil
      end
    end

    subject { TableauTrustedInterface::Report.new() }
  ...
  context 'when there is no view_server or server in the configuration' do
    before do
      TableauTrustedInterface.configure do |config|
        config.auth_server = 'http://example.org'
        config.default_view_server = nil
        config.default_server = nil
      end
    end
